...
from dqn.city import create_city   # here
...
directory = "/home/ubuntu/model_files2"
...
class DQN(object):
    ...
    def choose_action(self, _s, _actions, epsilon):
        values = []
        for a in _actions:
            x = torch.unsqueeze(torch.tensor(_s + a, dtype=torch.float).to(device), 0)  # add dimension at 0
            values.append(self.eval_net.forward(x).data.cpu().numpy().flatten()[0])
    ...
    def choose_action_max(self, _s, actions):
        values = []
        for a in actions:
            x = torch.unsqueeze(torch.tensor(np.append(_s, a), dtype=torch.float).to(device), 0)  # add dimension at 0
            values.append(self.target_net.forward(x).data.cpu().numpy().flatten()[0])
    ...
    def learn(self):
        ...
        state_action = torch.tensor(b_memory[:, :DIM_STATE+DIM_ACTION], dtype=torch.float).to(device)
        reward = torch.tensor(b_memory[:, DIM_STATE+DIM_ACTION:DIM_STATE+DIM_ACTION+1], dtype=torch.float).to(device)
        detach_state = torch.tensor(b_memory[:, -DIM_STATE-DIM_ACTION-1:-1], dtype=torch.float).to(device)
        done = torch.tensor(b_memory[:, -1], dtype=torch.float).to(device)
...
if __name__ == '__main__':
    ...
    for episode in range(80):
        ...
        episode_reward = 0
        busy_drivers = {}
        # driver: [state, action, reward, _state, _action]
        ...
        while env.city_time < end_time:
            ...
            for driver, [(loc, time), orders, drivers] in states.items():
                ...
                for nei_grid in neighbours:
                    reposition_duration = CONST["reposition_time"]
                ...
                if len(orders):
                    ...
                    if orders[aid].order_id is not None:
                        assert orders[aid].order_id not in selected_order
                        selected_order.add(orders[aid].order_id)
                    # For check use.
                    ###################################################################################################
                    # if driver == TARGET_DRIVER_ID:
                    #     # Get unique node id
            ...
            if dqn.memory_counter > BATCH_SIZE:
                ...
                dqn.learn()
            # if env.city_time % 3600 == 0:
        print("Episode: ", episode)
        print("Epsilon", epsilon)
        print("Total number of actions inside episode: ", count)
        print("Episode reward", episode_reward)
        print("Response rate", 1 - env.expired_order / env.n_orders)
        print(env.city_time, env.expired_order, env.n_orders)

        torch.save(dqn.eval_net.state_dict(), directory + "/eval_net_"+str(episode))
