...
myCity = CityReal(all_grids, "2016/11/01 10:00:00", False, False, order_num_dist, transition_prob_dict, transition_trip_time_dict, transition_reward_dict,
    ...
    while True:
        ...
        for grid_id, state in s.items():
            ...
            for i in range(pair_length):
                driver = drivers[i]
                order = orders[i]
                action.append([driver.node.get_node_index(), driver.get_driver_id(), order.get_begin_position().get_node_index(), order.order_id, None])
            # for order in orders:
            #     order.print_order()
